...
import { fetchSpecificQueries, processQueryData, clearDuneCache, CRYPTO_QUERIES } from '../services/duneService';
import './AnalyticsPage2.css';
...
const AnalyticsPage = () => {
  ...
  const fetchAnalyticsData = async () => {
    setLoading(true);
    try {
      const data = await fetchSpecificQueries(CRYPTO_QUERIES);
      setQueries(data);
  ...
  const renderChart = (query) => {
    ...
    if (chartData.type === 'line') {
      ...
      const doughnutOptions = {
        ...options,
        scales: undefined, // Remove scales for doughnut
        plugins: {
          ...options.plugins,
          legend: {
            position: 'right',
    ...
    <div className="analytics-page">
      <div className="analytics-header">
        <div className="header-content">
          ...
          <h1>Dune Analytics Dashboard</h1>
          <p className="subtitle">On-chain data insights powered by Dune Analytics</p>
        </div>
              ...
              return (
                <div key={queryId} className="query-card">
                  <div className="query-header">
                    <h3>{query.name}</h3>
                    <span className="query-id">Query #{queryId}</span>
                  </div>
                  ...
                  {query.error ? (
                    ...
                    <div className="chart-container">
                      {query.processed?.chartData?.needsDualAxis && (
                        <div className="dual-axis-indicator">Dual Y-Axis</div>
                      )}
                      {renderChart(query)}
                    </div>
                  )}
